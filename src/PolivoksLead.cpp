...
struct PolivoksLeadContainer leadsamples[ 36 ] = {
...
struct PolivoksLeadContainer *leadGetNote(float current) {
  for (int i = 0; i < 36; i++) {
...
struct PolivoksLeadModule : Module {
  ...
  PolivoksLeadModule( ) : Module(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS) {
    currentStep = 0;
    last = -1;
  }
...
PolivoksLeadWidget::PolivoksLeadWidget( ) {
  PolivoksLeadModule *module = new PolivoksLeadModule( );
  setModule(module);
