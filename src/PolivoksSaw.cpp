...
struct PolivoksSawContainer sawsamples[ 36 ] = {
...
struct PolivoksSawContainer *sawGetNote(float current) {
  for (int i = 0; i < 36; i++) {
...
struct PolivoksSawModule : Module {
  ...
  PolivoksSawModule( ) : Module(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS) {
    currentStep = 0;
    last = -1;
  }
...
PolivoksSawWidget::PolivoksSawWidget( ) {
  PolivoksSawModule *module = new PolivoksSawModule( );
  setModule(module);
